...
def run_pair(pair):
    ...
    print("\n--> Running", pair)
    #Multiple parameters for MACD, Signal Lines
    #Essentially running Guru Tester for each parameter and then added results to a dataframe
    for slow in [26,52]:
        for fast in [12,18]:
            if slow <= fast:
                continue
    ...
    for p1 in l_curr:
        for p2 in l_curr:
            ...
            if pair in ic.instruments_dict.keys():
                pairs.append(pair) #Getting list of viable unique pairs
    return pairs
...
def run_ema_macd(ic: InstrumentCollection):
    #Take in specified number of processes at once
    pairs = get_sim_pairs(['USD', 'GBP', 'JPY', 'NZD', 'AUD', 'CAD'], ic)
    
    #4 processors in parallel, so processes 4 pairs at once
    limit = 4
    current = 0
    ...
    while current < len(pairs):
        ...
        #Creates list of processes (4 processes in paralle)
        for _ in range(limit):
            #Create new process - completely separate execution to current running program
            #Target = function we want to execute in parallel
            processes.append(Process(target=run_process, args=(pairs[current],)))
            current +=1
        #Executes processes simutaneuously in processes
        for p in processes:
            p.start()
        #Wait until processes have finished running 
        #and then we're going to end the program that has created these processes
        for p in processes:
            p.join()
    print("ALL DONE")