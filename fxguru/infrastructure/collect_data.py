...
def save_file(final_df: pd.DataFrame, file_prefix, granularity, pair):
    ...
    print(f"*** {s1} --> {final_df.shape[0]} candles loaded ***")

def fetch_candles(pair, granularity, date_f: dt.datetime, date_t: dt.datetime, 
                  api: OandaApi):
    attempts = 0
    while attempts < 3:
