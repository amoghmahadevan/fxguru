...
def ATR(df: pd.DataFrame, n=14):
    ...
    #Creating Dataframe with 3 columns, and getting maximum value from each row and returning as series
...
def KeltnerChannels(df: pd.DataFrame, n_ema=20, n_atr=10):
    ...
    df = ATR(df, n=n_atr)
    c_atr = f"ATR_{n_atr}"
    df['KCUp'] = df.EMA + df[c_atr]*2 
    df['KCLo'] = df.EMA - df[c_atr]*2
    df.drop(c_atr, axis=1, inplace=True)
    return df

def RSI(df: pd.DataFrame, n=14):
    alpha = 1.0 / n #Smoothing factor
    gains = df.mid_c.diff()
